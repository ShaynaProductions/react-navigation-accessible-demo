...
import {getCommonTestElements, getSubNavTestElements} from "../utilities/renderedItems";
...
const renderNavigation = ({label, ...rest}: Omit<NavigationProps, "children">) => {
    return render(
        <Box cx="simple">
...
describe("Navigation simple keyboard handling", () => {
    ...
    it("4.2.1.5.2 - should move into it's child when the list is expanded.", async () => {
        ...
        const {aboutLink, readButton, readList, storiesLink} = getSubNavTestElements(getByRole, getByTestId, TEST_ID);
    ...
    it("4.2.1.5.1.2 should move to its topmost parent when the last item is a button with a" +
        ...
        const {aboutLink, readButton, readList, referenceButton, storiesLink} = getSubNavTestElements(getByRole, getByTestId, TEST_ID);
